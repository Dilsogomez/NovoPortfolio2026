        ...
        <section className="min-h-screen pt-28 pb-20 px-4 bg-gray-900 dark:bg-black text-white relative">
            ...
            <div className="max-w-7xl mx-auto mb-12 text-center relative z-10">
                ...
                <h1 className="text-4xl md:text-6xl font-extrabold font-montserrat mb-6 text-white">
                    Espaço de <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500">Criação</span>
                </h1>
            ...
            <div className="max-w-5xl mx-auto mb-16 relative z-20">
                <div className="flex flex-wrap justify-center gap-2 p-1.5 bg-white/5 border border-white/10 rounded-3xl backdrop-blur-md">
                    {[
            ...
            <div className="max-w-6xl mx-auto min-h-[500px] animate-fade-in-up">
                        ...
                        {PROJECTS.map((project, index) => (
                            <div key={index} className="group relative bg-gray-800/20 border border-white/10 rounded-3xl p-8 md:p-12 backdrop-blur-sm hover:border-blue-500/30 transition-all duration-500">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                                    ...
                                    <div className="relative w-full min-h-[300px] bg-black/60 rounded-2xl border border-white/10 overflow-hidden flex flex-col items-center justify-center group-hover:shadow-[0_0_30px_rgba(0,102,255,0.1)] transition-shadow">
                                        ...
                                        <div className="text-center p-8 relative z-10">
                                            ...
                                            <h3 className="text-lg font-bold text-white mb-2">Preview Interativo</h3>
                                            <p className="text-gray-500 text-xs max-w-xs mx-auto mb-4">
                                                Demonstração do projeto em tempo real.
                                            </p>
                                            <button className="px-5 py-2 bg-blue-600/20 text-blue-400 border border-blue-500/50 rounded-lg text-xs hover:bg-blue-600 hover:text-white transition-all uppercase font-bold tracking-wider">
                                                Carregar
                                            </button>
                                        </div>
                ...
                {activeTab === 'artigos' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {POSTS.map((post) => (
                            <article key={post.id} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-blue-500/30 hover:bg-white/[0.07] transition-all group flex flex-col">
                                <div className="h-48 overflow-hidden relative">
                                    <img src={post.imageUrl} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                                    <div className="absolute top-4 left-4 bg-black/60 backdrop-blur px-3 py-1 rounded-full border border-white/10 text-xs font-bold uppercase">
                                        {post.category}
                                    </div>
                                </div>
                                <div className="p-6 flex flex-col flex-grow">
                                    <span className="text-blue-400 text-xs font-mono mb-2">{post.date}</span>
                                    ...
                                    <button className="text-white text-sm font-semibold border-b border-blue-500 pb-1 self-start hover:text-blue-400 transition-colors">
                                        Ler Artigo Completo
                                    </button>
                                </div>
                            </article>
                ...
                {activeTab === 'videos' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {LAB_VIDEOS.map((video) => (
                            <div key={video.id} className="group bg-black border border-white/10 rounded-2xl overflow-hidden">
                                <div className="relative aspect-video bg-gray-900 overflow-hidden">
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <button className="w-14 h-14 rounded-full bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center text-white group-hover:scale-110 group-hover:bg-blue-600 group-hover:border-blue-500 transition-all duration-300">
                         ...
                         <div className="border border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center p-8 text-center text-gray-500 bg-white/5 min-h-[200px]">
                ...
                {activeTab === 'imagens' && (
                    <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
                        {LAB_IMAGES.map((img) => (
                            <div key={img.id} className="break-inside-avoid relative group rounded-2xl overflow-hidden border border-white/10 bg-gray-900">
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                                    ...
                                    {img.prompt && (
                                        <p className="text-gray-400 text-xs mt-3 italic line-clamp-2 border-l-2 border-blue-500 pl-2">
                                            "{img.prompt}"
                                        </p>
                                    )}
                                </div>
                ...
                {activeTab === 'estudos' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {LAB_STUDIES.map((study) => (
                            <div key={study.id} className="bg-gradient-to-br from-gray-800/40 to-black border border-white/10 rounded-2xl p-8 hover:border-purple-500/30 transition-all hover:-translate-y-1">
                                <div className="flex justify-between items-start mb-6">
                                    <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border ${
                                        study.status === 'Concluído' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 
                                        study.status === 'Em Andamento' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 
                                ...
                                <p className="text-gray-400 text-sm leading-relaxed mb-8">{study.description}</p>
                                <div className="flex flex-wrap gap-2 mt-auto">
                                    {study.tags.map((tag, i) => (
                                        <span key={i} className="text-xs text-purple-300 bg-purple-900/20 px-2 py-1 rounded border border-purple-500/10">
                                            #{tag}
                                        </span>
                                    ))}
                                </div>
                ...
                {activeTab === 'solicitar' && (
                    <div className="max-w-4xl mx-auto">
                        <div className="bg-gradient-to-br from-gray-800/50 to-black/80 border border-white/10 rounded-3xl p-8 md:p-12 backdrop-blur-xl relative overflow-hidden">
                            ...
                            <div className="text-center mb-10 relative z-10">
                                <h2 className="text-3xl font-bold text-white mb-4">Inicie seu <span className="text-blue-500">Próprio Projeto</span></h2>
                                <p className="text-gray-400">Selecione o tipo de serviço que você precisa e envie uma solicitação direta para o laboratório.</p>
                            </div>
                                ...
                                {serviceOptions.map((service) => (
                                    ...
                                    >
                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 text-xl ${
                                            selectedService === service.label ? 'bg-white text-blue-600' : 'bg-gray-800 text-gray-400 group-hover:text-white'
                                        }`}>
                                            <i className={service.icon}></i>
                                        </div>
                                        <h3 className={`font-bold mb-1 ${selectedService === service.label ? 'text-white' : 'text-gray-200'}`}>
                                            {service.label}
                                        </h3>
                            ...
                            <div className="space-y-6 relative z-10">
                                <div>
                                    <label className="block text-sm font-medium text-gray-400 mb-2">Detalhes da Solicitação</label>
                                    <textarea
                                        value={description}
                                        onChange={(e) => setDescription(e.target.value)}
                                        placeholder="Descreva brevemente o que você precisa..."
                                        className="w-full h-32 bg-black/40 border border-white/10 rounded-xl p-4 text-white placeholder-gray-600 focus:outline-none focus:border-blue-500 transition-colors resize-none"
                                    ></textarea>
                                </div>
